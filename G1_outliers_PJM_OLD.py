...
if os.path.isfile(outputdir+'/'+args.coh1+args.coh2+'_WG_'+str(args.snps)+'SNPs_3metrics.txt') == False:
    if os.path.isfile(outputdir+'/'+args.coh1+args.coh2+'_scaf_8_AFs.table') == False:
        ...
        for i in range(len(intable)):
            infile = open(intable[i],'r')
            infile.readline()
            outfile = open(outputdir+'/'+args.coh1+args.coh2+'_scaf_'+str(i+1)+'_AFs.table','w')
        ...
        outfile = open(outputdir+'/'+args.coh1+args.coh2+'_metrics_WG_'+str(args.snps)+'SNPs.txt','w')
        outfile.write(header)

        AFs = []
        for dirName, subdirList, fileList in os.walk(outputdir):
            for file in fileList:
                if file.startswith(args.coh1+args.coh2) and file.endswith('_AFs.table') == True:
                    AFs.append(dirName+'/'+file)
        AFs_sort = natsorted(AFs)

        print('\nFound '+str(len(AFs_sort))+' files starting with '+args.coh1+args.coh2+' and ending with _AFs.table\n')
        ...
        for dirName, subdirList, fileList in os.walk(outputdir):
            for file in fileList:
                if file.startswith(args.coh1+args.coh2) and file.endswith(str(args.snps)+'SNPs.txt'):
        ...
        for file in inlist:
            ...
            snp_file['DD'] = snp_file[args.coh1+'_pi'] - snp_file['prediction']

            snp_file.to_csv(outputdir+'/'+args.coh1+args.coh2+'_WG_'+str(args.snps)+'SNPs_3metrics.txt',sep="\t",index=False)
            print(slope,intercept)
            # rfile = open(outputdir+'/'+args.coh1+args.coh2+'_'+str(args.snps)+'.r', 'w')
            # rfile.write('d <- read.table("'+file+'", header=T)\n'+
            #             '# calculate DD-residuals\n'+
            #             'ddm <- lm('+args.coh1+'_pi~absdiff, data=d)\n'+
            #             'd$DD <- ddm$residuals\n'+
            #             'write.table(d, file="'+outputdir+'/'+args.coh1+args.coh2+'_WG_'+str(args.snps)+'SNPs_3metrics.txt", quote=F, row.names=F, sep="\t")')
            # rfile.close()

            # cmd = ('Rscript '+outputdir+'/'+args.coh1+args.coh2+'_'+str(args.snps)+'.r')
            # p = subprocess.Popen(cmd, shell=True)
            # sts = os.waitpid(p.pid, 0)[1]

            count += 1
            os.remove(outputdir+'/'+args.coh1+args.coh2+'_'+str(args.snps)+'.r')
        print('Processed '+str(count)+' files for DDresiduals')
        os.remove(outputdir+'/'+args.coh1+args.coh2+'_metrics_WG_'+str(args.snps)+'SNPs.txt')
        ...
        for dirName, subdirList, fileList in os.walk(outputdir):
            for file in fileList:
                if file.startswith(args.coh1+args.coh2) and file.endswith(str(args.snps)+'SNPs_3metrics.txt') == True:
        ...
        for out in out_sort:
            ...
            outlier_values = open(outputdir+'/'+args.coh1+args.coh2+'_'+str(args.snps)+'SNPs_'+str(args.ci)+'sd_outlier_descriptive_stats.txt', 'w')
            ...
            df_dat.to_csv(outputdir+'/'+args.coh1+args.coh2+'_'+str(args.snps)+'SNPs_'+str(args.ci)+'sd_allsites.csv', index=False)
            ...
            df_outlier.to_csv(outputdir+'/'+args.coh1+args.coh2+'_'+str(args.snps)+'SNPs_'+str(args.ci)+'sd_ALL_outliers.csv', index=False)
            # write bedfile for gene retrieval and orthologous gene onthology match
            header = ["scaffold", "bedstart", "end"]
            df_outlier.to_csv(outputdir+'/'+args.coh1+args.coh2+'_'+str(args.snps)+'SNPs_'+str(args.ci)+'sd_ALL_outliers.bed', index=False, sep='\t', columns=["scaffold", "start", "end"], header=False)
            ...
            if df_dxy.empty:
                ...
                df_dxy.to_csv(outputdir+'/'+args.coh1+args.coh2+'_'+str(args.snps)+'SNPs_'+str(args.ci)+'sd_Dxy_outliers.csv', index=False)
                # write bedfile for gene retrieval and orthologous gene onthology match
                header = ["scaffold", "bedstart", "end"]
                df_dxy.to_csv(outputdir+'/'+args.coh1+args.coh2+'_'+str(args.snps)+'SNPs_'+str(args.ci)+'sd_Dxy_outliers.bed', index=False, sep='\t', columns=["scaffold", "start", "end"], header=False)
            ...
            if df_fst.empty:
                ...
                df_fst.to_csv(outputdir+'/'+args.coh1+args.coh2+'_'+str(args.snps)+'SNPs_'+str(args.ci)+'sd_Fst_outliers.csv', index=False)
                # write bedfile for gene retrieval and orthologous gene onthology match
                header = ["scaffold", "bedstart", "end"]
                df_fst.to_csv(outputdir+'/'+args.coh1+args.coh2+'_'+str(args.snps)+'SNPs_'+str(args.ci)+'sd_Fst_outliers.bed', index=False, sep='\t', columns=["scaffold", "start", "end"], header=False)
            ...
            if df_dd.empty:
                ...
                df_dd.to_csv(outputdir+'/'+args.coh1+args.coh2+'_'+str(args.snps)+'SNPs_'+str(args.ci)+'sd_DD_outliers.csv', index=False)
                # write bedfile for gene retrieval and orthologous gene onthology match
                header = ["scaffold", "bedstart", "end"]
                df_dd.to_csv(outputdir+'/'+args.coh1+args.coh2+'_'+str(args.snps)+'SNPs_'+str(args.ci)+'sd_DD_outliers.bed', index=False, sep='\t', columns=["scaffold", "start", "end"], header=False)
            ...
            if df_dxyfst.empty:
                ...
                df_dxyfst.to_csv(outputdir+'/'+args.coh1+args.coh2+'_'+str(args.snps)+'SNPs_'+str(args.ci)+'sd_DxyFst_outliers.csv', index=False)
                # write bedfile for gene retrieval and orthologous gene onthology match
                header = ["scaffold", "bedstart", "end"]
                df_dxyfst.to_csv(outputdir+'/'+args.coh1+args.coh2+'_'+str(args.snps)+'SNPs_'+str(args.ci)+'sd_DxyFst_outliers.bed', index=False, sep='\t', columns=["scaffold", "start", "end"], header=False)
            ...
            if df_dxydd.empty:
                ...
                df_dxydd.to_csv(outputdir+'/'+args.coh1+args.coh2+'_'+str(args.snps)+'SNPs_'+str(args.ci)+'sd_DxyDD_outliers.csv', index=False)
                # write bedfile for gene retrieval and orthologous gene onthology match
                header = ["scaffold", "bedstart", "end"]
                df_dxydd.to_csv(outputdir+'/'+args.coh1+args.coh2+'_'+str(args.snps)+'SNPs_'+str(args.ci)+'sd_DxyDD_outliers.bed', index=False, sep='\t', columns=["scaffold", "start", "end"], header=False)
            ...
            if df_fstdd.empty:
                ...
                df_fstdd.to_csv(outputdir+'/'+args.coh1+args.coh2+'_'+str(args.snps)+'SNPs_'+str(args.ci)+'sd_FstDD_outliers.csv', index=False)
                # write bedfile for gene retrieval and orthologous gene onthology match
                header = ["scaffold", "bedstart", "end"]
                df_fstdd.to_csv(outputdir+'/'+args.coh1+args.coh2+'_'+str(args.snps)+'SNPs_'+str(args.ci)+'sd_FstDD_outliers.bed', index=False, sep='\t', columns=["scaffold", "start", "end"], header=False)
            ...
            if df_dxyfstdd.empty:
                ...
                df_dxyfstdd.to_csv(outputdir+'/'+args.coh1+args.coh2+'_'+str(args.snps)+'SNPs_'+str(args.ci)+'sd_DxyFstDD_outliers.csv', index=False)
                # write bedfile for gene retrieval and orthologous gene onthology match
                header = ["scaffold", "bedstart", "end"]
                df_dxyfstdd.to_csv(outputdir+'/'+args.coh1+args.coh2+'_'+str(args.snps)+'SNPs_'+str(args.ci)+'sd_DxyFstDD_outliers.bed', index=False, sep='\t', columns=["scaffold", "start", "end"], header=False)
        ...
        for dirName, subdirList, fileList in os.walk(outputdir):
            for file in fileList:
                if file.startswith(args.coh1+args.coh2) and file.endswith(str(args.snps)+'SNPs_'+str(args.ci)+'sd_allsites.csv') == True:
        ...
        for file in graphs:
            count = 0
            rfile = open(outputdir+'/graphs/'+args.coh1+args.coh2+'_graphs.r', 'w')
            rfile.write('# load table\n'+
                        ...
                        '# export as pdf\n'+
                        'pdf(file="'+outputdir+'/graphs/'+args.coh1+args.coh2+'_'+str(args.snps)+'SNPs_Dxy_histogram.pdf")\n'+
                        'p.dxy\n'+
                        'dev.off()\n'+
                        'pdf(file="'+outputdir+'/graphs/'+args.coh1+args.coh2+'_'+str(args.snps)+'SNPs_Fst_histogram.pdf")\n'+
                        'p.fst\n'+
                        'dev.off()\n'+
                        'pdf(file="'+outputdir+'/graphs/'+args.coh1+args.coh2+'_'+str(args.snps)+'SNPs_DD_histogram.pdf")\n'+
                        'p.dd\n'+
                        'dev.off()\n'+
                        'pdf(file="'+outputdir+'/graphs/'+args.coh1+args.coh2+'_'+str(args.snps)+'SNPs_length_histogram.pdf")\n'+
                        'p.length\n'+
                        'dev.off()')
            rfile.close()
